...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 IDU finalization collateral
            //       will also cover the old 50 IDU finalization collateral.
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    CAmount nSubsidy = 0;
      ...
      if (nHeight <= 2000 && nHeight > 750) {
          nSubsidy = 2.1 * COIN;
      } else if (nHeight <= 5000 && nHeight > 2000) {
          nSubsidy = 5.25 * COIN;
      } else if (nHeight <= 10000 && nHeight > 10000) {
          nSubsidy = 10.5 * COIN;
