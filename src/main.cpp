...
#if defined(NDEBUG)
#error "INDIUM cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < 2) {
        ...
        nSubsidy = 4 * COIN;
    } else if (nHeight <= 5000 && nHeight > 1728) {
        nSubsidy = 5 * COIN;
    } else if (nHeight <= 10000 && nHeight > 4608) {
        nSubsidy = 7.5 * COIN;
    } else if (nHeight <= 15000 && nHeight > 8928) {
        nSubsidy = 8 * COIN;
  	} else if (nHeight <= 25000 && nHeight > 11808) {
          nSubsidy = 9 * COIN;
  	} else if (nHeight <= 50000 && nHeight > 16128) {
          nSubsidy = 10 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZPIVStake)
{
    ...
    if (nHeight <= 43200) {
        ...
        //When zPIV is staked, masternode only gets 2 IDU
        ret = 3 * COIN;
        if (isZPIVStake)
            ret = 2 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from INDIUM
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert IDU to zPIV
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // INDIUM: We use certain sporks during IBD, so check to see if they are
