...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::IDU);
    ...
    if (!settings.contains("nAnonymizeIndiumAmount"))
        settings.setValue("nAnonymizeIndiumAmount", 1000);

    nAnonymizeIndiumAmount = settings.value("nAnonymizeIndiumAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeIndiumAmount"))
        SoftSetArg("-anonymizeIndiumamount", settings.value("nAnonymizeIndiumAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in indium.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIndiumAmount:
            return QVariant(nAnonymizeIndiumAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIndiumAmount:
            nAnonymizeIndiumAmount = value.toInt();
            settings.setValue("nAnonymizeIndiumAmount", nAnonymizeIndiumAmount);
            emit anonymizeIndiumAmountChanged(nAnonymizeIndiumAmount);
            break;
        case CoinControlFeatures:
